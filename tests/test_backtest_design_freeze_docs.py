...
def test_phase_2f_8_predeclared_validation_protocol(
    freeze_text: str,
) -> None:
    _assert_tokens(
        freeze_text,
        (
            ...
            "validation decision tree after V2 static-normal failed the primary S&P objective",
            "Same-history V3/V4 retuning is not allowed without explicit approval and a new anti-overfit protocol",
            "V2 static-normal full-history local evidence remains in-sample candidate evidence only",
            "Clean promotion requires one of",
            "a predeclared holdout window",
            "an untouched validation window",
            "forward paper evidence after design choices are frozen",
            "Path A - Stop rules tuning",
            "Preserve V1/V2 results as research evidence",
            "do not continue allocator redesign",
            "Path B - Predeclared holdout protocol",
            "Select a validation window before any further model selection",
            "Freeze all candidate rules before evaluating that window",
            "Do not use validation results to iteratively retune",
            "Path C - Forward paper protocol",
            "Freeze design choices before observation",
            "Collect paper evidence prospectively",
            "No runtime deployment conclusion until forward evidence exists",
            "Path D - Conservative research off-ramp",
            "S&P/core passive allocation plus, at most, a small research sleeve",
            "research-process statement, not investment advice",
            "Minimum success criteria for any future clean validation",
            "net terminal wealth must exceed S&P 500 TR KRW-unhedged terminal wealth",
            "must beat implemented US60 static neutral baseline",
            "must beat product-relative v1 neutral static baseline",
            "must improve over V1 rules",
            "must pass dataset/NAV/frequency/static-baseline sanity checks",
            "lower drawdown alone is not success",
            "no deployment approval follows from in-sample evidence",
            "no investment recommendation follows from research evidence",
...
def test_phase_2f_10_forward_paper_evidence_contract_freeze(
    freeze_text: str,
) -> None:
    _assert_tokens(
        freeze_text,
        (
            ...
            "freezes the forward paper evidence contract selected by Phase 2f-9 Path B",
            "repaired full local history has already been used for V1 failure analysis, V2 design, V2 gate review, and post-V2 path selection",
            "prospective forward paper evidence collected after design choices are frozen",
            ...
            "implemented US60 static neutral baseline",
            "local_monthly_static_neutral_baseline_us60_kr20_gold15_cash5.v1",
            "product-relative v1 neutral static baseline",
            ...
            "no final pass/fail gate may be declared before the full 12 monthly observations are complete",
            "first fully prospective monthly decision/evaluation cycle after this contract is committed",
            "still requires 12 completed monthly observations",
            "do not shorten the window after seeing evidence",
            "candidate net terminal wealth must exceed S&P 500 TR KRW-unhedged terminal wealth over the predeclared forward observation window",
            "must beat implemented US60 static neutral comparator",
            "must beat product-relative v1 neutral static comparator",
...
def test_phase_2f_11_forward_paper_monthly_template_freeze(
    freeze_text: str,
) -> None:
    _assert_tokens(
        freeze_text,
        (
            ...
            "template is for sanitized monthly reporting only",
            "monthly report is PENDING until the full 12-month observation window is complete",
            "No monthly report can create deployment approval, investment recommendation, buy/sell recommendation, or runtime allocation change",
            ...
            "Underperformance before 12 completed observations is recorded but does not become final failure unless a predeclared blocker is triggered",
            "Final pass/fail review requires all 12 monthly observations",
            "Failure to beat the S&P primary benchmark over the full window means no project-level success",
            "Lower drawdown alone is not success",
            "Sanitization rules",
...
def test_phase_2f_12_forward_paper_operator_readiness_checklist_freeze(
    freeze_text: str,
) -> None:
    _assert_tokens(
        freeze_text,
        (
            ...
            "Phase 2f-12 freezes the operator readiness checklist before the first monthly forward evidence report is accepted",
            "does not authorize paper trading, live trading, runtime activation, startup smoke, daemon startup, or any order flow",
            "does not create deployment approval, investment recommendation, buy/sell recommendation, or runtime allocation change",
            ...
            "if data availability delays the first fully prospective monthly decision/evaluation cycle",
            "the first accepted report is still observation_index 1 of 12",
            "the window must still contain 12 completed observations",
            ...
            "paper/live/scout/broker/risk/order modules changed as part of evidence collection",
            ...
            "verify GitHub main is unchanged from the expected baseline unless a repo change is reported",
            ...
            "mark complete monthly reports as PENDING_FULL_WINDOW unless a blocker is present",
