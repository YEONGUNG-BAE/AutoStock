...
이 프로젝트의 **최우선 경제적 목표**는 장기(약 10년) 관점에서 **S&P 500 총수익(total
return) 매수-보유 대비 초과 성과**를 내는 것이다. 안전한 균형 배분 봇이 되는 것 자체가
목표가 아니다.

- **운영 안전**(operational safety: 페이퍼 기본, 라이브/주문 게이트, fail-closed, 비밀정보
  보호 등)은 **필수 전제이지만 그 자체로 충분하지 않다.** 안전하게 도는 것만으로는 목표가
  달성되지 않는다.
- **투자 방어성**(현금/금 상시 보유, 낮은 주식 베타 등)은 이 목표를 위해 **완화될 수 있다.**
  단, 운영 안전 게이트는 절대 완화하지 않는다.

상세 방향과 향후 지표/백테스트/Allocator v2 방향은 계획 문서를 참고한다:
[`docs/INVESTMENT_OBJECTIVE_BENCHMARK.md`](docs/INVESTMENT_OBJECTIVE_BENCHMARK.md)
— 구현 요구사항이 아니라 계획/향후 방향 문서다.
