...
| 구현 상태 | **1A FRED replay + 1B live-smoke + 2A generic PRICE replay + 2B yfinance PRICE live-smoke + 3A DART DISCLOSURE replay** — `ops/fetch_research_sources.py`; urllib only in `fred_http_client.py`; yfinance only in `price_live_client.py` (lazy) |
