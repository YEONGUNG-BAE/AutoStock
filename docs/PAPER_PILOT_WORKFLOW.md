...
- **3G4-3 factor input bundle workflow** wraps 3G4-2 with an operator-local bundle manifest (candidate pool + factor inputs + corp-code snapshot paths). Reviewable artifacts only; no live factor scoring.
- **3G4-2 factor-ranked mapping workflow** orchestrates 3G4-1 factor signals + 3G3-2 ranked mapping into reviewable universe/provider mapping artifacts (advisory metadata only).
...
- **3H0 docs-only checkpoint complete** — approved end-to-end operator path from discovered/ranked/factor-scored KR candidates through 3E combined context and Scout packet is documented ([§ 3H0](REAL_RESEARCH_SOURCE_INTAKE.md#3h0--operator-end-to-end-intake-guardrail-checkpoint)). Path remains **review-gated and non-trading**; no broker/PaperLoop/KIS out of scope.
- **3H1 preflight helper implemented** — `ops/preflight_kr_end_to_end_intake.py` validates existing manifest artifact paths and emits review-only follow-up command plan; does not fetch, smoke, mutate config, or trade.
...
- **3H6 validator optional validation report implemented** — `--report-out` / `--force`; compact audit JSON after successful validation only; no command execution.
- **3H7 operator handoff manifest implemented** — `ops/build_kr_end_to_end_handoff_manifest.py`; artifact path/sha256 integrity index only; no artifact bodies; no command execution.
- **3H8 handoff manifest verifier implemented** — `ops/verify_kr_end_to_end_handoff_manifest.py`; recomputes size/sha256 and validates recorded metadata; writes no files; no command execution.
- **3H9 handoff verifier schema hardening implemented** — exact-key schema lock; unknown top-level manifest keys and unknown artifact entry keys rejected at validate stage; no workflow behavior change.
- **3H10 handoff verifier optional path containment implemented** — optional `--base-dir`; canonical resolved-path containment; read-only default path writes no files.
- **3H11 handoff verifier optional verification report implemented** — `--verification-report-out` / `--force`; compact audit JSON after successful verification only; excludes artifact/manifest bodies; public verifier API remains read-only.
- **3H12 verification report output path containment implemented** — with `--base-dir`, `--verification-report-out` must resolve inside the same base; validate-stage rejection before write; no new success payload keys.
- **3H13 verification report schema self-validation implemented** — in-memory exact-key report validation before atomic write; invalid payloads never reach disk; `artifact_roles` reuses manifest role validator; no new success payload keys.
- **3H14 handoff manifest builder validate-before-commit implemented** — temp write → existing 3H8 verifier validation → atomic replace; invalid generated manifests never reach `--manifest-out`; no new CLI flags.
- **3H15 handoff manifest builder base-dir containment implemented** — optional `--base-dir` on builder; supplied artifact paths and `--manifest-out` must resolve inside base before read/write; 3H14 validate-before-commit calls verifier with same resolved base; manifest/API/CLI success key sets unchanged.
- **3H16 handoff bundle round-trip smoke implemented** — fixture-only no-exec API round-trip in `tests/test_kr_end_to_end_preflight.py`; preflight summary/plan/structured plan → validation report → handoff manifest builder/verifier with `base_dir`; no new operator command; no command execution.
...
- **3H20 CLI verifier tamper-rejection smoke implemented** — tampered 3H16-generated bundles rejected by in-process `verify_handoff_manifest_main([... "--json"])` (`rc == 1`; exact CLI error mode; stage mapping; no traceback/raw-body echo; report not written on failure); no subprocess; no new operator command.
- **3H21 handoff pipeline failure no-partial-output smoke implemented** — upstream validator/builder/verifier CLI failures leave no partial downstream artifacts or outside-base parents; `--force` preserves existing validation report when re-validation fails before write; safe JSON errors only; in-process CLI; no new operator command.
- **3H22 normalized handoff bundle reproducibility smoke implemented** — repeated API/CLI in-process round-trips in separate temp bundles compared via path/hash-independent normalization (semantic manifest/report contract equality; per-bundle containment; exact-key body-free; no-exec) in `tests/test_kr_end_to_end_preflight.py`; no subprocess; no new operator command; no command execution.
- **3H23 CLI stdout success payload contract smoke implemented** — four handoff CLIs run in-process with `--json`; compact/safe success payloads locked (exact mode/stage/status, exact key sets, body-free, no sensitive/trading fields, advisory-only preflight followup_commands, output containment) in `tests/test_kr_end_to_end_preflight.py`; no subprocess; no new operator command; no command execution.
- **3H24 CLI stdout known-error payload contract smoke implemented** — four handoff CLIs run in-process with `--json` on representative known-domain-error inputs; compact/safe error payloads locked (exact mode/stage/status, exact four-key set, safe message, no traceback/raw-body echo, no partial downstream outputs, no sensitive/trading field keys; preflight missing-manifest domain path only) in `tests/test_kr_end_to_end_preflight.py`; no subprocess; no new operator command; no command execution.
- **3H25 CLI stdout JSON channel discipline smoke implemented** — four handoff CLIs run in-process with `--json` on success and known-domain-error paths; clean single-object stdout locked (`raw_decode` consumes stripped stdout; no human prefix/suffix; no traceback; no stderr JSON payload; success outputs exist; known-error no-partial outputs; no-exec) in `tests/test_kr_end_to_end_preflight.py`; no subprocess; no new operator command; no command execution.
- **3H26 CLI argument-domain failure no-output smoke implemented** — validator/builder/verifier handoff CLIs run in-process with `--json` on argument-domain failures (`write` output-exists preservation, `validate` missing/not-dir base_dir no partial output, stable blank path `args` errors; exact four-key error payload; no traceback/sentinel echo) in `tests/test_kr_end_to_end_preflight.py`; no subprocess; no new operator command; no command execution.
- **3H27 CLI help/usage side-effect smoke implemented** — four handoff CLIs run in-process on argparse `--help` and `main([])` only (`SystemExit` 0/non-zero; human usage text; no traceback/JSON; empty `tmp_path`; no generated command execution) in `tests/test_kr_end_to_end_preflight.py`; no subprocess; no new operator command; no command execution.
- **3H28 CLI help/usage wording contract smoke implemented** — four handoff CLIs `--help`/`main([])` in-process checks for operator-critical flag discoverability, usage-without-traceback/JSON, and forbidden compound operational token absence in `tests/test_kr_end_to_end_preflight.py`; no full help snapshot; no subprocess; no new operator command; no command execution.
- **3H29 CLI non-JSON human output smoke implemented** — four handoff CLIs run in-process without `--json` on success and known-domain-error paths; safe human stdout/stderr locked (`rc == 0`/`1`; non-JSON; no traceback/raw-body echo; success artifact writes; known-error no-partial outputs; validator output-exists preservation; no-exec) in `tests/test_kr_end_to_end_preflight.py`; preflight conflicting-flags path excluded; no subprocess; no new operator command; no command execution.
