...
| 구현 상태 | **1A FRED replay + 1B live-smoke + 2A generic PRICE replay + 2B yfinance PRICE live-smoke + 3A DART DISCLOSURE replay + 3A.1 Scout DART context** — `ops/fetch_research_sources.py`; DART disclosures in Scout packet when symbol matches scope (`market=None`; not 8C coverage) |
