...
| 구현 상태 | **1A–2B + 3A + 3A.1 + 3B1 + 3B2 + 3C1 + 3C2 + 3D1 + 3E1 + 3E2 + 3E3 + 3E4 + 3F1 + 3F2 + 3G1 + 3G2 + 3G3-0 + 3G3-1 + 3G3-2 + 3G3-3 + 3G3-4A + 3G3-4B + 3G3-5 + 3G3-6 + 3G4-0 + 3G4-1 + 3G4-2 + 3G4-3 + 3G4-4 + 3G4-5 + 3G4-H1** (factor intake hardening); **3H0** docs-only end-to-end operator path checkpoint **complete**; **3G4+** factor hardening deferred; **3G3-6+** source-specific live adapter hardening deferred; **3H1** manifest/preflight helper deferred |
...
- **3H0 docs-only checkpoint complete** — approved end-to-end operator path from discovered/ranked/factor-scored KR candidates through 3E combined context and Scout packet is documented ([§ 3H0](REAL_RESEARCH_SOURCE_INTAKE.md#3h0--operator-end-to-end-intake-guardrail-checkpoint)). Path remains **review-gated and non-trading**; no new orchestration command; broker/PaperLoop/KIS out of scope.
