...
- `docs/PAPER_FAST_LOOP_OPERATOR_APPROVAL_CONSUMPTION_ELIGIBILITY_ARTIFACT_PERSISTENCE_PAYLOAD_CONTRACT.md` (RTM-7c.4w — canonical persistence-payload encode/decode byte format + strict round-trip; API-only, no file I/O; decode VALID = consistency, not persistence/authenticity/provenance/consumption/activation)
