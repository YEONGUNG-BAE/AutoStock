...
- `docs/PAPER_FAST_LOOP_OPERATOR_APPROVAL_CONSUMPTION_ELIGIBILITY_ARTIFACT_FILE_CONTRACT.md` (RTM-7c.4x — atomic create-new file writer + read-only file reader; explicit caller path only; outcomes `WRITTEN`/`PUBLISHED_INCOMPLETE`/`PUBLICATION_UNCERTAIN`/`NOT_WRITTEN`/`INVALID`; resource-finalization closure separates temp-created, one-shot temp-fd-close, temp-cleanup, parent-sync-attempted/confirmed, and destination-published/uncertain state; fatal-cleanup closure gives operation fatal paths a single per-step-isolated cleanup owner; recovers temp-create/link side effects including hard-link-then-`EEXIST`; recovery-exception ownership closure uses structured recovery observations, operation > recovery > cleanup fatal precedence, link recovery exactly once, temp ordinary recovery uncertainty cleanup, and parent/destination unreadable preflight taxonomy; recovery-uncertainty closure treats only `ENOENT`/`ENOTDIR` as absent and preserves reader/directory fatal precedence; post-publish ordinary exceptions never become `NOT_WRITTEN`; no CLI; no consumption/replay/signing/activation; runtime activation NO-GO)
- `docs/PAPER_DAY_DIAGNOSTIC_RUNTIME_CONTRACT.md` / `docs/PAPER_DAY_DIAGNOSTIC_EVIDENCE_CONTRACT.md` / `docs/PAPER_DAY_OPERATOR_RUNBOOK.md` (RTM-7c.5a/5b — attended bounded paper-only diagnostic for `005930`; deterministic slow-loop publisher; evidence-first failure staging; not activation, not unattended, not live-order capable)
