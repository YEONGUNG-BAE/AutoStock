...
| **Safety** | `--force` applies only to `--manifest-out`; validate-before-write; atomic same-directory temp → replace |
| **Execution** | Does **not** execute plan commands, live fetches, smokes, or config mutation |
...
**Next deferred step — 3H14+ (unimplemented)**

**3H14+** — end-to-end hardening (calibration, drift checks, richer provenance) remains deferred. Broker/PaperLoop/KIS integration remains out of scope for Real Research Source Intake.
