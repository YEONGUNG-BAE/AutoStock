...
- Real Intake **3H1** — operator-local end-to-end manifest/preflight helper — Deferred: thin preflight over existing artifact paths; validate exist/parse + provider mapping coverage; optionally print follow-up commands referencing existing ops only; no live fetch; no checked-in config mutation; no broker/PaperLoop/KIS; **unimplemented**.
- Real Intake **3H2+** — end-to-end hardening (richer provenance, drift checks, orchestration ergonomics) — Deferred. Broker/PaperLoop/KIS remains out of scope for Real Research Source Intake.
