...
| `--verify-operator-approval-intent` | 0 if `VALID`, else 1 | stdin-only approval-intent schema + hash verification; **explicit** `--json`; no config/env/DB/fs write/clock read (RTM-7c.4q — see `PAPER_FAST_LOOP_OPERATOR_APPROVAL_INTENT_VERIFICATION_CONTRACT.md`) |
