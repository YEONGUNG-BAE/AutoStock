...
| `--build-operator-approval-intent` | 0 if combined PASS + intent CREATED, else 1 (`NO_GO`/`FAIL`) | stdin receipt + config (`environ={}`) + **required** `--max-age-microseconds` + three explicit manual confirmation flags (`--operator-approval-declared`, `--writers-stopped-manually-confirmed`, `--live-orders-forbidden-confirmed` — each must be present; absence is not approval); composes freshness-qualified evidence then `build_operator_approval_intent` once; stdout JSON only — no persistence/consumption/identity/signing/activation (RTM-7c.4p — see `PAPER_FAST_LOOP_OPERATOR_APPROVAL_INTENT_CLI_CONTRACT.md`) |
